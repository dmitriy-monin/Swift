...
let fio = (name : "Петр", patronymic : "Иванович", surname : "Петров")

switch fio {
case (let name,_,_) where name.hasPrefix("А") || name.hasPrefix("О") : print("Здравствуйте, \(fio.name)!")
case (_, let patronymic, _) where patronymic.hasPrefix("В") || patronymic.hasPrefix("Д") : print("Здравствуйте, \(fio.name) \(fio.patronymic)!")
case (_,_,let surname) where surname.hasPrefix("Е") || surname.hasPrefix("З") : print("Здравствуйте, \(fio.surname)!")
default:
    print("Здравствуйте, \(fio.surname) \(fio.name) \(fio.patronymic)!")
}

let (name, patronymic, surname) = ("Иван", "Петрович", "Вольнов")

// v.2 (Работает, но много вложенностей)
for i in name {
    switch i {
...
let field = (x: 1...10, y: 1...10)
let ship = (x: 3, y: 6, w: 0, h: 1)
let shot = (x: 3, y: 7)
